...
auto StackTracker::TryFindValidDisasm(uint64_t baseAddr, size_t maxOffset)
    ...
    for (size_t i = 0; i < maxOffset; ++i) {
        ...
        if (this->disasmCount > 0 && LookslikeValidEntry(testInsn, this->disasmCount)) {
            this->baseAddr += i;
            // NOTE: original code freed this->insn here and then copied
            // this->insn[j] -> use-after-free, corrupted insList and produced
            // false "Non-integrity" detections on real ntdll frames. Removed
            // the free; the destructor already comments out the same free.
...
StackTracker::StackTracker(HANDLE hProcess, uint64_t StartAddress,
    ...
    // FIX: ins_ip/ins_ip_address/readSuccess were never initialized; a garbage
    // ins_ip >= insList.size() made getNextIns() return nullptr immediately,
    // so CalcNextJmpAddress always failed and every scanned frame looked like
    // a "Non-integrity" stack (false positives on real ntdll/kernel32 frames).
    this->ins_ip = 0;
    this->ins_ip_address = 0;
