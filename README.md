...
**内容一致性维度（hollows_hunter v0.4.1.1 实测）**。本方案的 `.pay` 磁盘上是 StompTarget（30 个 noinline 调用），运行时被 stomp 成 shellcode——内存与磁盘永远不一致，这是本方案最根本的"内容层"暴露。但工具现实与直觉不同：

| 场景 | 默认 | `/obfusc 3` | `/threads` | `/hooks` |
|---|---|---|---|---|
| island（.pay stomp + XOR 窗口） | 0 | 0 | 0 | **0** |
| 私有 RWX shellcode（对照） | 0 | 0 | **1**（implanted_shc） | — |
| 教科书 .text 改写（对照） | 0 | — | — | **1** |

三个事实：① hollows_hunter 默认**不做节内容比对**（教科书 stomp 都放过）；② `/hooks` 的函数级比对只覆盖标准代码节导出/入口区，`.pay` 因非标准节名 + 无导出而豁免——**实现豁免，不是类别免疫**，全量比对的扫描器一旦出现，`.pay` 必死；③ `/obfusc 3` 对映像节内 0x15 字节 XOR 不触发（Moonwalk++ 评估里的检出场景是私有内存的大区域加密）——窗口方案把密文缩到 obfus 粒度之下是意外收益，不是设计。
